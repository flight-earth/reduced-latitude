type deg = { deg : Convert.deg }
type rad = { rad : Convert.rad }
type dms = { deg : int; min : int; sec : float }

let from_rad (r : rad) : deg = { deg = Convert.rad_to_deg r.rad }

let signum (x : float) : Convert.ordering =
  if x < 0.0 then Convert.Lt else if x > 0.0 then Convert.Gt else Convert.Eq

let sign_dms (x : dms) : Convert.ordering * dms =
  let sign =
    if x.deg < 0 || x.min < 0 || x.sec < 0.0 then Convert.Lt
    else if x.deg = 0 && x.min = 0 && x.sec = 0.0 then Convert.Eq
    else Convert.Gt
  in
...
let display_dms (x : dms) : string =
  string_of_int x.deg ^ "°" ^ string_of_int x.min ^ "′" ^ string_of_float x.sec
  ^ "″"
  ...
  from_deg
    {
      deg =
        (if b = 0.0 then
           if Convert.is_even (Int64.of_int (abs a)) then 0.0
           else (if a < 0 then -1.0 else 1.0) *. 180.0
         else if Convert.is_even (Int64.of_int (abs a)) then b
         else b -. 180.0);
    }
